...
import QuickMarkPeaksButton from "../components/QuickMarkPeaksButton";
import TripReportsList from "./TripReportsList";
import type { TripReportData } from "./TripReportsList";
import AvatarUpload from "./AvatarUpload";
import SummitHistoryGrid from "./SummitHistoryGrid";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <aside className="lg:col-span-3 space-y-4 lg:space-y-6 min-w-0">
            ...
            <div className="bg-white rounded-2xl border border-[var(--color-border-app)] p-4 sm:p-5">
              ...
              <nav className="space-y-1">
                ...
                <QuickMarkPeaksButton
                  peaks={allPeaks.map((p) => ({ id: p.id, name: p.name, elevation: p.elevation, range: p.range, rank: p.rank }))}
                  completedPeakIds={Array.from(completedPeakIds)}
                  summitLogs={(summitLogs || []).map((l) => ({ peak_id: l.peak_id, summit_date: l.summit_date }))}
                  isLoggedIn={!!user}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all group hover:bg-[var(--color-surface-subtle)]"
                >
                  <span className="flex-shrink-0 text-[var(--color-brand-primary)]"><ChecklistIcon /></span>
                  <span className="flex-1 min-w-0 text-sm font-medium text-[var(--color-text-primary)]">Quick Mark Peaks</span>
                </QuickMarkPeaksButton>
          ...
          <main className="lg:col-span-9 space-y-6 lg:space-y-8 min-w-0">
            ...
            <section className="bg-white rounded-2xl border border-[var(--color-border-app)] overflow-hidden">
              ...
              <SummitHistoryGrid peaks={completedPeaks} />
            </section>
