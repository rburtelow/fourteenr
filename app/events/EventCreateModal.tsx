...
export default function EventCreateModal({ isOpen, onClose, allPeaks, onEventCreated, groupId, groupName }: EventCreateModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Create Event" size="lg">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {groupName && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[var(--color-surface-subtle)] border border-[var(--color-border-app)] text-sm">
            <UsersGroupIcon className="w-4 h-4 text-[var(--color-brand-primary)] flex-shrink-0" />
            <span className="text-[var(--color-text-secondary)]">Posting to</span>
            <span className="font-semibold text-[var(--color-text-primary)]">{groupName}</span>
          </div>
        )}
